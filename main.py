...
# 채팅방 이름
kakaoRoomName = ["고1 쌤없 톡방", "1-5 (선생님)"]
...
while True:
    ...
    ok_hour = [i for i in range(start_hour, end_hour+1, 1)]
    send_min = 50 if hour < 13 else 40 # 점심 전까지는 50분에, 이후에는 40분에 안내
    message = "📢 현재 시간 {0}시 {1}분을 지나가고 있습니다.\n" \
              "1학년 5반 학생들은 금일 16시 40분까지 [베네듀 일일 과제]를 신속히 완료하시기 바랍니다.".format(hour, minute)

    for room in kakaoRoomName:
        if(hour in ok_hour and day in ok_day):
            if(minute == send_min):
                cnt = 0
